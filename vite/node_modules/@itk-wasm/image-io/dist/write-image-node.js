...
import { getFileExtension, castImage, } from 'itk-wasm';
 ...
 */
async function writeImageNode(image, serializedImage, options = {}) {
    ...
    if (typeof mimeType !== 'undefined' && mimeToImageIo.has(mimeType)) {
        ...
        for (const readerWriter of imageIoIndexNode.values()) {
            if (readerWriter[1] !== null) {
                let { couldWrite } = await readerWriter[1](inputImage, absoluteFilePath, { useCompression: options.useCompression });
                if (couldWrite) {
                    return;
                }
            }
    ...
    const readerWriter = imageIoIndexNode.get(io);
...
//# sourceMappingURL=write-image-node.js.map